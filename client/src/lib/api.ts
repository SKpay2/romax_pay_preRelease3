 ...
 */
export async function adminGetAllDeposits(password: string): Promise<AdminDeposit[]> {
  const response = await fetch(`${API_BASE}/admin/deposits/all?password=${encodeURIComponent(password)}`);
