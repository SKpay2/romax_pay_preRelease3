...
export default function AdminPanel() {
  ...
  const [paymentTab, setPaymentTab] = useState<'active' | 'completed'>('active');
  ...
  const loadDeposits = async () => {
    ...
    try {
      const data = depositTab === 'pending' 
        ...
        : await api.adminGetAllDeposits(adminPassword);
      setDeposits(data);
    } catch (error) {
      toast({
        title: 'Ошибка',
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <Tabs defaultValue="users" className="w-full">
          ...
          <TabsContent value="payments" className="space-y-4 mt-6">
            ...
            <Tabs value={paymentTab} onValueChange={(v) => setPaymentTab(v as 'active' | 'completed')} className="w-full">
              <TabsList className="grid w-full grid-cols-2 bg-card shadow-soft-sm rounded-[18px] p-1">
                <TabsTrigger 
                  value="active" 
                  className="rounded-[14px] data-[state=active]:bg-accent data-[state=active]:text-accent-foreground data-[state=active]:shadow-soft-sm font-semibold transition-soft"
                >
                  Активные ({payments.filter(p => ['submitted', 'assigned', 'processing'].includes(p.status)).length})
                </TabsTrigger>
                <TabsTrigger 
                  value="completed" 
                  className="rounded-[14px] data-[state=active]:bg-accent data-[state=active]:text-accent-foreground data-[state=active]:shadow-soft-sm font-semibold transition-soft"
                >
                  Завершенные ({payments.filter(p => ['paid', 'rejected', 'cancelled'].includes(p.status)).length})
                </TabsTrigger>
              </TabsList>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead>Оператор</TableHead>
                        <TableHead>Взято</TableHead>
                        <TableHead>Время обработки</TableHead>
                        <TableHead>Создана</TableHead>
                        <TableHead>Действия</TableHead>
                      </TableRow>
                    </TableHeader>
                      ...
                      {payments.length === 0 ? (
                        ...
                        payments.map((payment) => (
                          <TableRow key={payment.id}>
                            ...
                            <TableCell className="text-sm">
                              {payment.processingTimeMinutes !== null && payment.processingTimeMinutes !== undefined ? (
                                formatProcessingTime(payment.processingTimeMinutes)
                              ) : (
