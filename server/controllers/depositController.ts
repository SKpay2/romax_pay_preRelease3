...
import { notifyDepositConfirmed } from '../services/notificationService';
...
export async function createAutomatedDeposit(req: Request, res: Response) {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      console.log(`[createAutomatedDeposit] START (attempt ${attempt}/${MAX_RETRIES}) - userId: ${userId}, rawAmount: ${rawRequestedAmount}`);
      ...
      console.log(`[createAutomatedDeposit] Calling generateUniquePayableAmount for ${requestedAmount} USDT`);
      const payableAmount = await generateUniquePayableAmount(requestedAmount);
      console.log(`[createAutomatedDeposit] Generated payableAmount: ${payableAmount} USDT`);
      ...
      console.log(`[createAutomatedDeposit] Deposit created successfully with payableAmount: ${payableAmount}`);
      ...
      if (isUniqueConstraintViolation && attempt < MAX_RETRIES) {
        console.log(`[createAutomatedDeposit] Unique constraint violation on attempt ${attempt}, retrying...`);
        await new Promise(resolve => setTimeout(resolve, 50 * attempt));
        continue;
      }
      
      console.error(`[createAutomatedDeposit] Error on attempt ${attempt}:`, error);
...
export async function createDeposit(req: Request, res: Response) {
  try {
    ...
    await storage.createNotification({
      userId,
      message: `Создана заявка на депозит ${formatUsdtBalance(amount)} USDT. Ожидает подтверждения администратором.`,
      type: 'general',
      isRead: 0,
    });
...
export async function cancelDeposit(req: Request, res: Response) {
  try {
    ...
    await storage.createNotification({
      ...
      message: `Заявка на пополнение ${formatUsdtBalance(deposit.requestedAmount || deposit.amount)} USDT была отменена`,
      type: 'general',
      isRead: 0,
    });
...
export async function manualConfirmDeposit(req: Request, res: Response) {
  try {
    ...
    console.error('Error manually confirming deposit:', error);
