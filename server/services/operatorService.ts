...
export async function notifyOnlineOperators(paymentRequest: PaymentRequest): Promise<void> {
  try {
    if (!operatorBot) {
      console.error('Operator bot not initialized');
      return;
    }
    ...
    if (onlineOperators.length === 0) {
      console.log('No online operators to notify');
      return;
    }
    ...
    for (const operator of onlineOperators) {
      if (operator.chatId) {
        try {
          ...
          console.log(`Notification sent to operator ${operator.login} (chatId: ${operator.chatId})`);
        } catch (error) {
          console.error(`Failed to send notification to operator ${operator.login}:`, error);
        }
      }
    }
  } catch (error) {
    console.error('Error in notifyOnlineOperators:', error);
  }
}
  ...
  try {
    ...
    for (const chatId of operatorChatIds) {
      try {
        ...
        console.error(`Failed to notify operator at chatId ${chatId}:`, error);
      }
    }
  } catch (error) {
    console.error('Error in notifyOperatorTaskTaken:', error);
  }
}
